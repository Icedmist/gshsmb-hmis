...
import { Department, Pagination as PaginationType } from '../types';
...
export default function DepartmentsPage() {
  ...
  const [hospitals, setHospitals] = useState<{ id: number; hospital_name: string; hospital_code: string }[]>([]);
  ...
  const [form, setForm] = useState({ department_name: '', department_code: '', description: '', hospital_ids: [] as number[] });
  ...
  const loadDepartments = async (page = 1) => {
    ...
    try {
      ...
      const data: any = await api.get(`/departments?${params}`);
  ...
  const loadHospitals = async () => {
    try {
      const data: any = await api.get('/hospitals?limit=9999');
    ...
    try {
      if (editDept) {
        await api.put(`/departments/${editDept.id}`, {
          ...
          hospital_id: form.hospital_ids[0],
        });
      } else {
  ...
  const handleToggleStatus = async (d: Department) => {
    ...
    try {
      await api.put(`/departments/${d.id}`, { status: newStatus });
      loadDepartments(pagination.page);
    } catch (err: any) { alert(err.message); }
  };

  const hospitalMap = useMemo(() => {
    const m = new Map<number, typeof hospitals[0]>();
          ...
          <div>
            ...
            {editDept ? (
              <div className="relative">
                <select className="input appearance-none w-full" value={form.hospital_ids[0] || ''} onChange={e => setForm({ ...form, hospital_ids: [parseInt(e.target.value)] })} required>
