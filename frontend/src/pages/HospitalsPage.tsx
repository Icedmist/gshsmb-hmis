...
export default function HospitalsPage() {
  ...
  const loadHospitals = async (page = 1) => {
    ...
    try {
      ...
      const data: any = await api.get(`/hospitals?${params}`);
  ...
  const loadDepartmentNames = async () => {
    try {
      const data: any = await api.get('/departments/names');
      setExistingDeptNames(data.departmentNames || []);
    } catch {}
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editHospital) {
        await api.put(`/hospitals/${editHospital.id}`, form);
      } else {
        const created: any = await api.post('/hospitals', form);
        if (selectedDeptNames.length > 0) {
          const newHospitalId = created.id;
          for (const deptName of selectedDeptNames) {
            ...
            try {
              await api.post('/departments', {
                ...
                department_code: dept.base_code,
                description: dept.description || '',
                hospital_ids: [newHospitalId],
              });
            } catch {}
          }
        }
  ...
  const handleToggleStatus = async (h: Hospital) => {
    ...
    try {
      await api.put(`/hospitals/${h.id}`, { status: newStatus });
      loadHospitals(pagination.page);
    } catch (err: any) { alert(err.message); }
  };
