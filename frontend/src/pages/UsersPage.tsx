  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editUser) {
        ...
        if (form.hospital_id) payload.hospital_id = parseInt(form.hospital_id);
        await api.put(`/users/${editUser.id}`, payload);
      } else {
        await api.post('/users', { ...form, hospital_id: form.hospital_id ? parseInt(form.hospital_id) : undefined });
      }
      setShowModal(false);
