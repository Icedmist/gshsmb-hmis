...
import { Employee, EmployeeTransfer, Pagination as PaginationType } from '../types';
...
export default function EmployeesPage() {
  ...
  const [hospitals, setHospitals] = useState<{ id: number; hospital_name: string }[]>([]);
  const [departments, setDepartments] = useState<{ id: number; department_name: string; hospital_id: number }[]>([]);
  ...
  const loadEmployees = async (page = 1) => {
    ...
    try {
      ...
      const data: any = await api.get(`/employees?${params}`);
  ...
  const loadReferenceData = async () => {
    try {
      const [hData, dData] = await Promise.all([
        api.get<{ hospitals: any[] }>('/hospitals?limit=9999'),
        api.get<{ departments: any[] }>('/departments?limit=9999'),
      ]);
  ...
  const getFilteredDepartments = (hospitalId: string) => {
    return departments.filter(d => d.hospital_id === parseInt(hospitalId));
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const payload = { ...form, department_id: parseInt(form.department_id), hospital_id: parseInt(form.hospital_id) };
      if (editEmp) {
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/employees/${showTransfer.id}/transfer`, {
        to_hospital_id: parseInt(transferForm.to_hospital_id),
        to_department_id: parseInt(transferForm.to_department_id),
  ...
  const handleToggleStatus = async (e: Employee) => {
    ...
    try {
      await api.put(`/employees/${e.id}`, { status: newStatus });
      loadEmployees(pagination.page);
    } catch (err: any) { alert(err.message); }
  };
          ...
          <form onSubmit={handleSearch} className="flex gap-3 flex-wrap items-center">
            ...
            {hospitalFilter && (
              <span className="text-xs text-slate-400 ml-auto">
                Showing {pagination.total} employee{pagination.total !== 1 ? 's' : ''} at {hospitals.find(h => h.id === parseInt(hospitalFilter))?.hospital_name}
              </span>
            )}
          </form>
        </div>
