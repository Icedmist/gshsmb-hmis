  ...
  const loadDashboardData = useCallback(async (silent = false) => {
    ...
    try {
      const [s, eph, epd, re, rt, ra] = await Promise.all([
        ...
        api.get<any[]>('/dashboard/employees-per-hospital'),
        api.get<any[]>('/dashboard/employees-per-department'),
        api.get<any>('/dashboard/recent-employees'),
        api.get<any>('/dashboard/recent-transfers'),
        api.get<any>('/dashboard/recent-activities'),
      ]);
      setStats(s);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          trendValue={totalEmployees > 0 ? '+5%' : '0%'}
        />
        <StatCard
          ...
          icon={UserCheck}
          color="teal"
          subtitle={`${activePercent}% of total workforce`}
          trend={activePercent >= 80 ? 'up' : activePercent >= 50 ? 'neutral' : 'down'}
          trendValue={`${activePercent}% active`}
        />
      </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="w-full h-1.5 bg-slate-100 rounded-full mt-1 overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min((d.value / Math.max(...deptWithData.map(x => x.value))) * 100, 100)}%`,
