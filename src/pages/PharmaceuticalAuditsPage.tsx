...
export default function PharmaceuticalAuditsPage() {
  ...
  const [findingForm, setFindingForm] = useState({ audit_id: '', finding: '', severity: 'medium' as string, recommendation: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editItem) {
        await updatePharmaceuticalAudit(editItem.id, form);
      } else {
        await createPharmaceuticalAudit(form);
      }
      setShowModal(false);
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      await deletePharmaceuticalAudit(item.id);
      loadData(pagination.page);
    } catch (err: any) { alert(err.message); }
  };
  ...
  const handleToggleStatus = async (item: any) => {
    ...
    try {
      await updatePharmaceuticalAudit(item.id, { status: newStatus });
      loadData(pagination.page);
    } catch (err: any) { alert(err.message); }
  };
  ...
  const toggleFindings = async (auditId: string) => {
    ...
    if (!findings[auditId]) {
      ...
      try {
        const data = await getPharmaceuticalAuditFindings(auditId);
  ...
  const handleSubmitFinding = async (e: React.FormEvent) => {
    ...
    try {
      if (editFinding) {
        await updatePharmaceuticalAuditFinding(editFinding.id, findingForm);
      } else {
        await createPharmaceuticalAuditFinding(findingForm);
      }
      setShowFindingModal(false);
      const data = await getPharmaceuticalAuditFindings(findingForm.audit_id);
  ...
  const toggleFindingImplemented = async (finding: any) => {
    try {
      await updatePharmaceuticalAuditFinding(finding.id, { implemented: !finding.implemented, implemented_at: !finding.implemented ? new Date().toISOString() : null });
      const data = await getPharmaceuticalAuditFindings(finding.audit_id);
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
            ...
            <span className="text-slate-800 font-medium">Pharmaceutical Audits</span>
          </div>
          <h1 className="text-2xl font-bold text-slate-900">Pharmaceutical Audits</h1>
          <p className="text-slate-500 mt-1 text-sm">Conduct and manage pharmaceutical audits across hospitals</p>
        </div>
        {canManage && <button onClick={openCreate} className="btn-primary"><Plus size={18} /> Add Audit</button>}
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Audits" value={pagination.total} icon={ClipboardCheck} color="primary" subtitle="All pharmaceutical audits" />
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 text-sm">No pharmaceutical audits found.</p>
      ...
      <Modal open={showFindingModal} onClose={() => setShowFindingModal(false)} title={editFinding ? 'Edit Finding' : 'Add Finding'} size="md">
        <form onSubmit={handleSubmitFinding} className="space-y-4">
          ...
          <div>
            ...
            <select className="input" value={findingForm.severity} onChange={e => setFindingForm({ ...findingForm, severity: e.target.value })} required>
