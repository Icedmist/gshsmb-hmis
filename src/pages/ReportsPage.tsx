...
const reportFetchers: Record<string, () => Promise<any[]>> = {
  'workforce': getWorkforceDistribution,
  'hospital-staffing': getHospitalStaffing,
  'department-staffing': getDepartmentStaffing,
  'transfers': getTransfersReport,
  'active-employees': getActiveEmployeesReport,
};
...
export default function ReportsPage() {
  ...
  const canView = hasRole('super_admin', 'executive_secretary', 'hospital_admin', 'hr_officer', 'director_hr', 'director_prs', 'hospital_admin');
  ...
  const loadReport = async (key: string) => {
    ...
    try {
      const fetcher = reportFetchers[key];
      if (!fetcher) throw new Error(`Unknown report: ${key}`);
  ...
  const handleExport = (key: string) => {
    ...
    if (!rows || rows.length === 0) {
      alert('No data to export. View the report first.');
      return;
    }
