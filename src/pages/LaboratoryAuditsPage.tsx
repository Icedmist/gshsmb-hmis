...
import { getLaboratoryAudits, createLaboratoryAudit, updateLaboratoryAudit, deleteLaboratoryAudit, getLaboratoryAuditFindings, createLaboratoryAuditFinding, updateLaboratoryAuditFinding } from '../lib/laboratory';
import { getAllHospitals } from '../lib/hospitals';
import { getHospitalScope } from '../lib/scope';

export default function LaboratoryAuditsPage() {
  ...
  const loadData = async (page = 1) => {
    setLoading(true);
    try {
      const { data, total } = await getLaboratoryAudits(page, 50, search || undefined, hospitalScope);
      setItems(data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editItem) {
        await updateLaboratoryAudit(editItem.id, form);
      } else {
        await createLaboratoryAudit(form);
      }
      setShowModal(false);
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      await deleteLaboratoryAudit(item.id);
      loadData(pagination.page);
    } catch (err: any) { alert(err.message); }
  };
  ...
  const handleToggleStatus = async (item: any) => {
    ...
    try {
      await updateLaboratoryAudit(item.id, { status: newStatus });
      loadData(pagination.page);
    } catch (err: any) { alert(err.message); }
  };
  ...
  const toggleFindings = async (auditId: string) => {
    ...
    if (!findings[auditId]) {
      ...
      try {
        const data = await getLaboratoryAuditFindings(auditId);
  ...
  const handleSubmitFinding = async (e: React.FormEvent) => {
    ...
    try {
      if (editFinding) {
        await updateLaboratoryAuditFinding(editFinding.id, findingForm);
      } else {
        await createLaboratoryAuditFinding(findingForm);
      }
      setShowFindingModal(false);
      const data = await getLaboratoryAuditFindings(findingForm.audit_id);
      setFindings(prev => ({ ...prev, [findingForm.audit_id]: data }));
    } catch (err: any) { alert(err.message); }
  };

  const toggleFindingImplemented = async (finding: any) => {
    try {
      await updateLaboratoryAuditFinding(finding.id, { implemented: !finding.implemented, implemented_at: !finding.implemented ? new Date().toISOString() : null });
      const data = await getLaboratoryAuditFindings(finding.audit_id);
      setFindings(prev => ({ ...prev, [finding.audit_id]: data }));
    } catch (err: any) { alert(err.message); }
  };

  const completedCount = items.filter(a => a.status === 'inactive').length;
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
            ...
            <span className="text-slate-800 font-medium">Laboratory Audits</span>
          </div>
          <h1 className="text-2xl font-bold text-slate-900">Laboratory Audits</h1>
          <p className="text-slate-500 mt-1 text-sm">Conduct and manage laboratory audits across hospitals</p>
        </div>
        {canManage && <button onClick={openCreate} className="btn-primary"><Plus size={18} /> Add Audit</button>}
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Audits" value={pagination.total} icon={ClipboardCheck} color="primary" subtitle="All laboratory audits" />
        <StatCard title="Active" value={items.filter(a => a.status === 'active').length} icon={ClipboardList} color="teal" subtitle="Open audits" />
        <StatCard title="Completed" value={completedCount} icon={CheckCircle} color="blue" subtitle="Closed audits" />
        <StatCard title="Hospitals Covered" value={new Set(items.map(a => a.hospital_id)).size} icon={Building2} color="army" subtitle="Hospitals audited" />
      </div>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 text-sm">No laboratory audits found.</p>
