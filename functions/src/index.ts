...
export const createUser = functions.https.onCall(async (data: CreateUserRequest, context: functions.https.CallableContext) => {
  ...
  const validRoles = ['super_admin', 'executive_secretary', 'hospital_admin', 'hr_officer', 'director_hr', 'director_medical_services', 'director_nursing_services', 'director_prs', 'director_pharmaceutical_services', 'director_laboratory_services', 'director_finance'];
  if (!validRoles.includes(data.role)) {
    throw new functions.https.HttpsError('invalid-argument', 'Invalid role.');
  }

  const hospitalScopedRoles = ['hospital_admin', 'hr_officer'];
